...
public class HomeActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
                                                ...
                                                for (Map.Entry<String, Subject> entry1 : subjectCodesAndNames.entrySet()) {
                                                    ...
                                                    FirebaseDatabase.getInstance().getReference("students_data/" + user.getUid() + "/subjects/" + entry1.getKey())
                                                            .addListenerForSingleValueEvent(new ValueEventListener() {
                                                                ...
                                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                                    if (!snapshot.exists()) {
                                                                        snapshot.getRef().child("feedbackSubmitted").setValue(false);
    ...
    protected void onDestroy() {
        if (valueEventListener != null) {
            FirebaseDatabase.getInstance().getReference("/attendance/active_attendance/" + completeDivisionName).removeEventListener(valueEventListener);
        }
        if (valueEventListener2 != null) {
            FirebaseDatabase.getInstance().getReference("/attendance/active_attendance/" + completeBatchName).removeEventListener(valueEventListener2);
        }
        super.onDestroy();
    }
